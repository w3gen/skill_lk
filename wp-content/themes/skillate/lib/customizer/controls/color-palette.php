...
if( class_exists( 'WP_Customize_Control' ) && !class_exists( 'SKILLATE_THMC_Color_Palette_Control' ) ):
	/**
	* 
	*/
	class SKILLATE_THMC_Color_Palette_Control extends WP_Customize_Control
	{
		...
		public function render_content() {
			...
			<div class="thmc-color-palettes clearfix">
				<?php foreach ($this->choices as $input_value => $skillate_colors): ?>
					<div class="thmc-color-palette">
						...
						<label for="<?php echo esc_attr( $name.'_'.$input_value ); ?>">
							<?php
							foreach ($skillate_colors as $skillate_color) {
								?>
								<span style="background-color:<?php echo esc_attr($skillate_color); ?>;"></span>
								<?php
							}
							?>
						</label>
