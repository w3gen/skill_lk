...
<?php	
	...
	foreach($pmpro_levels_filtered as $level)
	{
		...
		if(isset($current_user->membership_level->ID))
		  ...
		  $current_level = false;
	?>
	<div id="pmpro_level-<?php echo $level->id; ?>" class="pmpro_level col-md-4 <?php 
		if($template === "genesis") {
					...
					if(!empty($show_price))
					{
						...
						<?php
							if(pmpro_isLevelFree($level))
							{
								...
								{
									?>
									<strong><?php _e('Free.', 'skillate'); ?></strong>
									<?php
								}
								else
								{	
				...
				<?php 
					...
					{ 
						...
						if(empty($level_expiration))
							_e('Membership Never Expires.', 'skillate');
						else
			...
			<?php if($layout === 'div' || $layout === '2col' || empty($layout)) { ?>
				...
				else
				{
					...
					<div<?php if($template != "twentyfourteen") { ?> class="entry-content"<?php } ?>>
						...
						?>
						<h2 class="subscription-price">
							<?php echo $pmpro_currency_symbol.$level->billing_amount; ?>
						</h2>
						<span class="period-text">Per <?php echo $level->cycle_period; ?></span>
						<?php
